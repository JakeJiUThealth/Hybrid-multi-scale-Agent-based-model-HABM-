...
#define MAXTreg 3 // Cell cycle in M stage
...
{
 public:
  int TregApopTimer; // the timer that records the apoptosis time steps of the PC cell
  int TregWaitProTimer; // the timer that records the waiting time of proliferation
  int TregCycleTimer; // the timer that records the time of Cell cycle
  int TregCurrentTimer; // the timer that records the current time of the cell
  int TregLx,TregLy,TregLz;// The location of it

  int TregApopStatus;// apoptosis status: Absorbed ->PCAPOPA;  Being apoptosis ->PCAPOPY; No->PCAPOPN
  int TregCellStatus; //The four status of cell cycle: G0/G1 ->StaG01;  S ->StaS;  G2 ->StaG2;  M ->StaM
					// if the value is StaN-> did not enter cell cycle
  int TregFreeSpace; // Whether there are free spaces: Yes->PCFREESPAY  No->PCFREESPAN
  int TregGenNo;
  Treg (int,int,int,int);
  virtual ~Treg();
